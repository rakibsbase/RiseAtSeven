"use client";

import React, { useState, useEffect, useRef } from "react";
import { FiArrowUpRight } from "react-icons/fi";
import Link from "next/link";
import gsap from "gsap";

// Navigation schema defining the multi-column mega menu structures and preview assets.
const navItems = [
  {
    id: "102",
    label: "Services",
    hasDropdown: true,
    columns: [
      {
        title: "Core Services",
        links: [
          {
            label: "Search & Growth Strategy",
            href: "/services/strategy-growth",
            id: 4790,
            image: "/navbar/Screenshot-2025-06-23-at-23.14.49.webp",
          },
          {
            label: "Onsite SEO",
            href: "/services/onsite-seo",
            id: 11981,
            image: "/navbar/WhatsApp-Image-2025-06-03-at-08.34.50.webp",
          },
          {
            label: "Content Experience",
            href: "/services/content-experience",
            id: 4789,
            image: "/navbar/Screenshot-2025-06-23-at-23.16.14.webp",
          },
          {
            label: "B2B Marketing",
            href: "/services/b2b-marketing",
            id: 22669,
            image: "/navbar/0B5A6875.webp",
          },
        ],
      },
      {
        title: "",
        links: [
          {
            label: "Digital PR",
            href: "/services/digital-pr",
            id: 12019,
            image: "/navbar/Screenshot-2025-06-23-at-22.39.35.webp",
          },
          {
            label: "Social Media & Campaigns",
            href: "/services/social",
            id: 12020,
            image:
              "/navbar/temp_image_43CEDE6C-4430-479F-9DBF-B348FA9AC991.webp",
          },
          {
            label: "Data & Insights",
            href: "/services/data-insights",
            id: 12021,
            image: "/navbar/data.webp",
          },
          {
            label: "Social SEO/Search",
            href: "/services/social-seo-tiktok-youtube",
            id: 16559,
            image: "/navbar/Screenshot-2025-09-24-at-11.47.25.webp",
          },
        ],
      },
    ],
  },
  {
    id: "23929",
    label: "Industries",
    hasDropdown: true,
    columns: [
      {
        title: "",
        links: [
          {
            label: "B2B Marketing",
            href: "/services/b2b-marketing",
            id: 23931,
            image: "/navbar/0B5A6875.webp",
          },
        ],
      },
    ],
  },
  {
    id: "103",
    label: "International",
    hasDropdown: true,
    columns: [
      {
        title: "",
        links: [
          {
            label: "US Digital PR",
            href: "/international/us-digital-pr",
            id: 4762,
            image: "/navbar/d4df0d30-d590-4e94-9056-9491f4beacba.webp",
          },
          {
            label: "Spain Digital PR",
            href: "/spain-digital-pr",
            id: 23207,
            image: "/navbar/Logos_2026-04-23-101020_frxy.webp",
          },
          {
            label: "Germany Digital PR",
            href: "/germany-digital-pr",
            id: 23208,
            image: "/navbar/27.webp",
          },
          {
            label: "Netherlands Digital PR",
            href: "/netherlands-digital-pr",
            id: 23603,
            image: "/navbar/Logos_2026-04-23-095313_xfhk.webp",
          },
        ],
      },
    ],
  },
  {
    id: "16913",
    label: "About",
    hasDropdown: true,
    columns: [
      {
        title: "",
        links: [
          {
            label: "About Us",
            href: "/about",
            id: 16915,
            image: "/navbar/0B5A7487.webp",
          },
          {
            label: "Meet The Risers",
            href: "/meet-the-team",
            id: 16916,
            image: "/navbar/Screenshot-2025-06-23-at-23.14.49.webp",
          },
          {
            label: "Culture",
            href: "/culture",
            id: 16917,
            image: "/navbar/Screenshot-2025-06-23-at-23.14.49.webp",
          },
          {
            label: "Testimonials",
            href: "/testimonials",
            id: 16918,
            image: "/navbar/d4df0d30-d590-4e94-9056-9491f4beacba.webp",
          },
        ],
      },
    ],
  },
  { label: "Work", href: "/work", badge: "25" },
  { label: "Careers", href: "/careers" },
  { label: "Blog", href: "/blog" },
  { label: "Webinar", href: "/webinars" },
];

const Navbar = () => {
  // --- UI State Management ---
  const [mobileMenu, setMobileMenu] = useState(false);
  const [mobileAccordion, setMobileAccordion] = useState(null);
  const [megaMenu, setMegaMenu] = useState(false); // Stores ID of active mega menu panel
  const [activePreviewId, setActivePreviewId] = useState(null); // Active link being hovered in mega menu
  const [hideHeader, setHideHeader] = useState(false);
  const [hideHeaderBackground, setHideHeaderBackground] = useState(true);

  // --- Animation & Persistence Refs ---
  const previousScrollPosition = useRef(0);
  const previousMegaMenuWidth = useRef(null);
  const previousMegaMenuHeight = useRef(null);
  const hoverBackgroundRef = useRef(null);
  const navContainerRef = useRef(null);
  const megaMenuRefs = useRef({});
  const closeTimeoutRef = useRef(null);
  const lastNavItemRectRef = useRef(null);

  const scheduleClose = () => {
    closeTimeoutRef.current = setTimeout(() => {
      setMegaMenu(false);
      gsap.to(hoverBackgroundRef.current, { opacity: 0, duration: 0.3 });
    }, 150);
  };

  const cancelClose = () => {
    clearTimeout(closeTimeoutRef.current);
  };

  const megaMenuLayoutById = {
    102: {
      panelWidthClass: "lg:w-[948px]",
      linkTextClass: "text-xl",
      imageWrapClass: "w-80",
      showImageCta: true,
      panelPaddingClass: "p-3",
      contentPaddingClass: "px-12 py-8",
    },
    23929: {
      panelWidthClass: "lg:w-[571px]",
      linkTextClass: "text-3xl",
      imageWrapClass: "w-72",
      showImageCta: false,
      panelPaddingClass: "p-3",
      contentPaddingClass: "px-14 py-10",
    },
    103: {
      panelWidthClass: "lg:w-[664px]",
      linkTextClass: "text-3xl",
      imageWrapClass: "w-72",
      showImageCta: false,
      panelPaddingClass: "p-3",
      contentPaddingClass: "px-14 py-10",
    },
    16913: {
      panelWidthClass: "lg:w-[586px]",
      linkTextClass: "text-3xl",
      imageWrapClass: "w-72",
      showImageCta: false,
      panelPaddingClass: "p-3",
      contentPaddingClass: "px-14 py-10",
    },
  };

  // Synchronize initial visibility states for GSAP panels.
  useEffect(() => {
    Object.values(megaMenuRefs.current).forEach((menu) => {
      if (menu)
        gsap.set(menu, { scaleX: 1, opacity: 0, pointerEvents: "none" });
    });
  }, []);

  // --- Mega Menu Orchestration ---
  // Handles layout-aware scaling and alpha transitions for sub-navigation panels.
  useEffect(() => {
    if (megaMenu) {
      const activeMenu = megaMenuRefs.current[megaMenu];
      if (!activeMenu) return;

      const newRect = activeMenu.getBoundingClientRect();
      const newWidth = newRect.width;
      const newHeight = newRect.height;

      // Calculate scale factors to prevent harsh jumps between menus of different sizes.
      const fromWidth = previousMegaMenuWidth.current || newWidth;
      const fromHeight = previousMegaMenuHeight.current || newHeight;

      gsap.killTweensOf(activeMenu);
      gsap.set(activeMenu, {
        transformOrigin: "top center",
        scaleX: fromWidth / newWidth,
        scaleY: fromHeight / newHeight,
        opacity: 1,
        pointerEvents: "auto",
      });

      gsap.to(activeMenu, {
        scaleX: 1,
        scaleY: 1,
        duration: 0.6,
        ease: "expo.out",
      });

      // Cull background animations for inactive panels.
      Object.entries(megaMenuRefs.current).forEach(([id, menu]) => {
        if (id !== megaMenu && menu) {
          gsap.killTweensOf(menu);
          gsap.set(menu, {
            opacity: 0,
            scaleX: 1,
            scaleY: 1,
            pointerEvents: "none",
          });
        }
      });

      previousMegaMenuWidth.current = newWidth;
      previousMegaMenuHeight.current = newHeight;
      document.body.classList.add("overflow-hidden");
    } else {
      document.body.classList.remove("overflow-hidden");
      Object.values(megaMenuRefs.current).forEach((menu) => {
        if (menu)
          gsap.to(menu, { opacity: 0, duration: 0.2, pointerEvents: "none" });
      });
      previousMegaMenuWidth.current = null;
      previousMegaMenuHeight.current = null;
    }
  }, [megaMenu]);

  // --- Scroll Logic ---
  // Optimized scroll listener that avoids unnecessary re-renders by utilizing derived direction logic.
  useEffect(() => {
    const handleScroll = () => {
      const currentScroll = window.scrollY;
      const prevScroll = previousScrollPosition.current;

      if (currentScroll > 100) {
        setHideHeaderBackground(false);
        // Hide header on down-scroll, restore on up-scroll for UX efficiency.
        if (currentScroll > prevScroll) {
          setHideHeader(true);
        } else {
          setHideHeader(false);
        }
      } else {
        // Force header visibility and transparency at top-of-page.
        setHideHeaderBackground(true);
        setHideHeader(false);
      }

      previousScrollPosition.current = currentScroll;
    };

    window.addEventListener("scroll", handleScroll, { passive: true });
    return () => window.removeEventListener("scroll", handleScroll);
  }, []);

  // --- Hover interaction helper ---
  // Manages the sliding pill background for desktop navigation items.
  const updateHoverBackground = (event, value, force = false) => {
    if (
      value &&
      event &&
      hoverBackgroundRef.current &&
      navContainerRef.current
    ) {
      const targetRect = event.currentTarget.getBoundingClientRect();
      const menuRect = navContainerRef.current.getBoundingClientRect();

      gsap.to(hoverBackgroundRef.current, {
        width: targetRect.width,
        left: targetRect.left - menuRect.left,
        duration: 0.3,
        ease: "power2.out",
        opacity: 1,
      });
    } else if (!value || force) {
      gsap.to(hoverBackgroundRef.current, {
        opacity: 0,
        duration: 0.3,
      });
    }
  };

  return (
    <div className="w-full relative z-70">
      {/* Primary Sticky Container */}
      <div
        className={`w-full fixed top-0 left-0 z-50 flex transition-transform duration-700 h-18 | lg:h-22 lg:p-3 ${
          hideHeader ? "-translate-y-full" : "translate-y-0"
        }`}
      >
        {/* Full-screen backdrop to trap focus and handle exit on mega-menu open */}
        <div
          className={`fixed inset-0 w-screen h-screen z-10 transition-all duration-400 pointer-events-none ${
            megaMenu ? "backdrop-blur-lg pointer-events-auto bg-black/20" : ""
          }`}
          onMouseEnter={() => {
            setMegaMenu(false);
          }}
        />

        {/* Inner Nav Bar: Transitions from transparent at top to solid/blurred on scroll */}
        <div
          className={`w-full flex items-center justify-between relative z-20 px-4 transition-all duration-500 | lg:px-6 lg:rounded-full ${
            hideHeaderBackground
              ? "translate-y-10"
              : "translate-y-0 bg-white/80 backdrop-blur-xl shadow-lg"
          }`}
        >
          {/* Logo */}
          <Link
            href="/"
            className={`flex w-32 ml-2 | md:w-40 transition-colors duration-300 ${
              hideHeaderBackground ? "text-white" : "text-black"
            }`}
          >
            <svg
              className="w-full h-full object-contain fill-current"
              viewBox="0 0 168 21"
            >
              <path d="M91.3152 5.40061C91.3152 3.94241 92.5306 2.67359 93.9881 2.67359C95.7162 2.67359 96.797 3.83419 96.797 5.56225H99.7127C99.7127 2.1873 97.3096 0 93.9874 0C90.9371 0 88.3988 2.32257 88.3988 5.42766C88.3988 9.31596 90.883 10.2344 93.9874 11.4221C95.6627 12.07 97.2007 12.5563 97.2007 14.6895C97.2007 16.634 95.9867 18.0651 93.9874 18.0651C91.8813 18.0651 90.7477 16.3905 90.7477 14.446H87.832C87.832 18.0651 90.3426 20.7381 93.9874 20.7381C97.6323 20.7381 100.118 18.2816 100.118 14.6895C100.118 7.10161 91.3145 9.64061 91.3145 5.40061H91.3152Z"></path>
              <path d="M109.209 4.99609C104.834 4.99609 101.539 8.53405 101.539 12.8539C101.539 17.1737 104.888 20.738 109.155 20.738C112.422 20.738 115.203 18.713 116.337 15.662H113.529C112.718 17.2278 111.017 18.1733 109.262 18.1733C106.806 18.1733 104.915 16.4182 104.348 14.0963H116.743C116.797 13.6371 116.823 13.1508 116.823 12.6922C116.823 8.47926 113.447 4.99609 109.209 4.99609ZM104.348 11.9361C104.509 9.47823 106.751 7.56147 109.181 7.56147C111.611 7.56147 113.853 9.47823 114.014 11.9361H104.348Z"></path>
              <path d="M127.476 5.40039L123.575 16.0941L119.673 5.40039H116.676L122.617 20.3598H124.588L130.475 5.40039H127.476Z"></path>
              <path d="M137.942 4.99609C133.567 4.99609 130.273 8.53405 130.273 12.8539C130.273 17.1737 133.621 20.738 137.888 20.738C141.155 20.738 143.936 18.713 145.071 15.662H142.262C141.453 17.2278 139.75 18.1733 137.996 18.1733C135.538 18.1733 133.649 16.4182 133.081 14.0963H145.476C145.53 13.6371 145.556 13.1508 145.556 12.6922C145.556 8.47926 142.182 4.99609 137.942 4.99609ZM133.081 11.9361C133.243 9.47823 135.484 7.56147 137.915 7.56147C140.347 7.56147 142.586 9.47823 142.749 11.9361H133.081Z"></path>
              <path d="M147.473 8.21195V8.69013V20.3618H150.032V10.1815L167.216 20.3618V17.2405L147.473 5.40039V8.21195Z"></path>
              <path d="M67.8431 7.50804H67.789C66.6818 5.80635 64.7103 4.99609 62.713 4.99609C58.1775 4.99609 54.7734 8.3981 54.7734 12.935C54.7734 17.4719 58.2296 20.7387 62.713 20.7387C64.7651 20.7387 66.7359 19.8473 67.789 18.0387H67.8431V20.3606H70.652V5.40122H67.8431V7.50804ZM62.686 18.1733C59.823 18.1733 57.5823 15.7168 57.5823 12.9073C57.5823 10.0978 59.7425 7.56079 62.7124 7.56079C65.6822 7.56079 67.8972 9.90973 67.8972 12.9073C67.8972 15.9048 65.6024 18.1733 62.6867 18.1733H62.686Z"></path>
              <path d="M77.5832 0.378906H74.7736V5.40144H72.75V7.96681H74.7736V20.3608H77.5832V7.96681H80.0403V5.40144H77.5832V0.378906Z"></path>
              <path d="M18.3089 0.378906H15.5V3.2953H18.3089V0.378906Z"></path>
              <path d="M18.3089 5.02344H15.5V19.9828H18.3089V5.02344Z"></path>
              <path d="M25.8409 10.7205C24.8142 10.3959 23.5183 10.0996 23.5183 8.77603C23.5183 7.77639 24.3279 7.18256 25.2728 7.18256C26.4077 7.18256 27.0549 7.91166 27.1895 8.99178H29.9984C29.9443 6.39935 27.9727 4.61719 25.4087 4.61719C22.8447 4.61719 20.7088 6.3723 20.7088 8.93767C20.7088 14.2307 27.5412 12.6102 27.5412 15.743C27.5412 17.0389 26.6227 17.7951 25.381 17.7951C23.707 17.7951 22.9516 16.6074 22.8427 15.0681H20.0352C20.0352 17.417 21.1951 19.2269 23.4094 20.0094C24.0303 20.2252 24.6789 20.3604 25.3262 20.3604C28.1892 20.3604 30.3494 18.5248 30.3494 15.5807C30.3494 12.6366 28.296 11.476 25.8402 10.7205H25.8409Z"></path>
              <path d="M39.3637 4.61719C34.9891 4.61719 31.6953 8.15514 31.6953 12.475C31.6953 16.7948 35.0432 20.3591 39.3096 20.3591C42.577 20.3591 45.3581 18.3341 46.493 15.2831H43.6842C42.8746 16.8489 41.1722 17.7944 39.4178 17.7944C36.96 17.7944 35.0709 16.0393 34.5028 13.7174H46.8975C46.9516 13.2582 46.978 12.7719 46.978 12.3133C46.978 8.10036 43.6037 4.61719 39.3637 4.61719ZM34.5028 11.5565C34.6651 9.09864 36.9059 7.18188 39.3373 7.18188C41.7688 7.18188 44.0075 9.09932 44.1705 11.5565H34.5028Z"></path>
              <path d="M9.55945 12.1512C12.1519 11.2327 13.3395 9.09953 13.3395 6.39957C13.3395 4.67151 12.7728 2.88934 11.5046 1.67395C10.0998 0.297591 8.07419 0 6.18314 0H0V19.9826H2.91572V13.8069L13.3389 19.9826V16.8606L6.22575 12.5949L7.61496 12.5293C8.26222 12.5293 8.96359 12.3676 9.55809 12.1512H9.55945ZM4.91499 10.3156H2.91572V2.67359H5.99444C8.317 2.67359 10.4231 3.86192 10.4231 6.40024C10.4231 9.5865 7.50742 10.3156 4.91499 10.3156Z"></path>
              <path d="M164.759 7.94414L166.061 8.71517V8.08955L165.395 7.69051C165.437 7.68172 165.48 7.66954 165.521 7.65466C165.869 7.53157 166.061 7.24209 166.061 6.84034C166.061 6.57725 165.966 6.33579 165.801 6.17753C165.583 5.9638 165.277 5.93945 165.065 5.93945H164.191V8.63807H164.758V7.94346L164.759 7.94414ZM164.908 7.22856H164.76V6.47715H165.043C165.261 6.47715 165.495 6.57251 165.495 6.84102C165.495 7.10953 165.297 7.22856 164.908 7.22856H164.908Z"></path>
              <path d="M165.127 10.1622C166.714 10.1622 168 8.87583 168 7.28913C168 5.70242 166.714 4.41602 165.127 4.41602C163.54 4.41602 162.254 5.70242 162.254 7.28913C162.254 8.87583 163.54 10.1622 165.127 10.1622ZM165.127 5.22763C166.264 5.22763 167.189 6.15219 167.189 7.28913C167.189 8.42606 166.264 9.35062 165.127 9.35062C163.99 9.35062 163.066 8.42606 163.066 7.28913C163.066 6.15219 163.99 5.22763 165.127 5.22763Z"></path>
            </svg>
          </Link>

          {/* --- Navigation Links --- */}
          <nav
            ref={navContainerRef}
            className="hidden lg:flex items-center relative gap-x-1 py-1 px-1 rounded-full group/nav"
            onMouseLeave={() => {
              scheduleClose();
            }}
          >
            {/* Sliding Pill Background: Visual anchor for active hover state */}
            <div
              ref={hoverBackgroundRef}
              className={`absolute h-[calc(100%-8px)] top-1 bg-gray-100 rounded-full transition-opacity duration-300 pointer-events-none opacity-0`}
            />

            {navItems.map((item) => (
              <div key={item.label} className="static">
                <Link
                  href={item.href || "#"}
                  className={`group relative z-20 inline-flex items-center px-4 py-2 text-sm font-semibold tracking-tight transition-colors duration-300 rounded-full ${
                    hideHeaderBackground && megaMenu !== item.id
                      ? "text-white"
                      : "text-gray-900"
                  } hover:text-gray-900`}
                  onMouseEnter={(e) => {
                    cancelClose();
                    lastNavItemRectRef.current =
                      e.currentTarget.getBoundingClientRect();
                    updateHoverBackground(e, true);
                    if (item.hasDropdown) {
                      setMegaMenu(item.id);
                      const firstLink = item.columns?.flatMap(
                        (c) => c.links,
                      )?.[0];
                      if (firstLink) setActivePreviewId(firstLink.id);
                    } else {
                      setMegaMenu(false);
                    }
                  }}
                >
                  {item.label}
                  {item.hasDropdown && (
                    <span className="hidden ml-1 pointer-events-none md:inline-block opacity-40 group-hover:opacity-100 transition-opacity">
                      +
                    </span>
                  )}
                  {item.badge && (
                    <span className="absolute top-0 right-0 translate-x-1/4 -translate-y-1/4 px-1.5 py-0.75 rounded-full bg-[#B2F6E3] text-gray-900 text-[9px] font-bold leading-none shadow-sm border border-white/20 transition-transform duration-300 group-hover:-translate-y-1/2">
                      {item.badge}
                    </span>
                  )}
                </Link>
              </div>
            ))}
          </nav>

          {/* --- Call to Action --- */}
          <div className="flex items-center gap-x-4">
            <Link
              href="/connect-with-us"
              className={`hidden lg:flex group relative px-6 py-3 rounded-full font-bold transition-all duration-300 overflow-hidden ${
                hideHeaderBackground
                  ? "bg-white text-gray-900"
                  : "bg-gray-900 text-white"
              } hover:rounded-xl`}
            >
              <div className="relative overflow-hidden h-6">
                <div className="flex items-center gap-2 transition-transform duration-300 group-hover:-translate-y-8">
                  Get in touch <FiArrowUpRight />
                </div>
                <div className="absolute top-0 left-0 flex items-center gap-2 translate-y-8 transition-transform duration-300 group-hover:translate-y-0">
                  Get in touch <FiArrowUpRight />
                </div>
              </div>
            </Link>

            {/* Mobile Navigation Trigger */}
            <button
              className={`lg:hidden p-2 transition-colors duration-300 ${
                hideHeaderBackground ? "text-white" : "text-black"
              }`}
              onClick={() => setMobileMenu(!mobileMenu)}
            >
              <div className="w-6 flex flex-col gap-1.5 items-end">
                <div className="h-0.5 w-6 bg-current" />
                <div className="h-0.5 w-6 bg-current" />
              </div>
            </button>
          </div>

          {/* --- Mega Menu Dropdown Panels --- */}
          {navItems.map(
            (item) =>
              item.hasDropdown && (
                <div
                  key={`mega-${item.id}`}
                  ref={(el) => (megaMenuRefs.current[item.id] = el)}
                  className={`absolute top-[calc(100%+12px)] left-1/2 -translate-x-1/2 bg-white rounded-[40px] shadow-[0_30px_100px_rgba(0,0,0,0.15)] flex w-[calc(100vw-2rem)] max-w-[calc(100vw-2rem)] ${
                    megaMenuLayoutById[item.id]?.panelWidthClass || "lg:w-190"
                  } lg:max-w-none overflow-hidden ${
                    megaMenuLayoutById[item.id]?.panelPaddingClass || "p-3"
                  } border border-gray-100 z-50 pointer-events-none opacity-0`}
                  onMouseEnter={() => {
                    cancelClose();
                    setMegaMenu(item.id);
                    if (lastNavItemRectRef.current && navContainerRef.current) {
                      const rect = lastNavItemRectRef.current;
                      const menuRect =
                        navContainerRef.current.getBoundingClientRect();
                      gsap.to(hoverBackgroundRef.current, {
                        width: rect.width,
                        left: rect.left - menuRect.left,
                        opacity: 1,
                        duration: 0.3,
                        ease: "power2.out",
                      });
                    }
                  }}
                  onMouseLeave={() => {
                    scheduleClose();
                  }}
                >
                  {/* Links Content Column */}
                  <div
                    className={`flex-1 min-w-0 flex items-center justify-center gap-x-12 ${
                      megaMenuLayoutById[item.id]?.contentPaddingClass ||
                      "px-12 py-8"
                    }`}
                  >
                    {item.columns.map((col, idx) => (
                      <div key={idx} className="flex-1 -mt-1">
                        <h4 className="text-gray-400 text-base font-medium tracking-tight mb-4 h-8">
                          {col.title}
                        </h4>
                        <div className="flex flex-col gap-y-0.5">
                          {col.links.map((link) => (
                            <Link
                              key={link.label}
                              href={link.href}
                              className="group/link block py-1"
                              onMouseEnter={() => setActivePreviewId(link.id)}
                            >
                              <div className="relative overflow-hidden truncate">
                                {/* Vertical Text-Slide Hover Effect */}
                                <div
                                  className={`${
                                    megaMenuLayoutById[item.id]
                                      ?.linkTextClass || "text-3xl"
                                  } font-medium leading-tight tracking-tight truncate whitespace-nowrap transition-transform duration-500 cubic-bezier(0.16, 1, 0.3, 1) group-hover/link:-translate-y-8`}
                                >
                                  {link.label}
                                </div>
                                <div
                                  className={`${
                                    megaMenuLayoutById[item.id]
                                      ?.linkTextClass || "text-3xl"
                                  } font-medium leading-tight tracking-tight truncate whitespace-nowrap absolute top-0 left-0 translate-y-8 transition-transform duration-500 cubic-bezier(0.16, 1, 0.3, 1) group-hover/link:translate-y-0`}
                                >
                                  {link.label}
                                </div>
                              </div>
                            </Link>
                          ))}
                        </div>
                      </div>
                    ))}
                  </div>

                  {/* Contextual Image Preview Panel */}
                  <div
                    className={`${
                      megaMenuLayoutById[item.id]?.imageWrapClass || "w-72"
                    } relative shrink-0 m-3`}
                  >
                    <div className="relative rounded-3xl overflow-hidden aspect-square bg-gray-900">
                      {item.columns
                        .flatMap((c) => c.links)
                        .map((link) => (
                          <div
                            key={link.id}
                            className={`absolute inset-0 transition-all duration-1000 cubic-bezier(0.16, 1, 0.3, 1) ${
                              activePreviewId === link.id
                                ? "opacity-100 scale-100"
                                : "opacity-0 scale-110 blur-xl"
                            }`}
                          >
                            <img
                              src={link.image}
                              alt={link.label}
                              className="w-full h-full object-cover"
                            />
                          </div>
                        ))}
                    </div>
                    {megaMenuLayoutById[item.id]?.showImageCta && (
                      <div className="absolute bottom-0 left-0 w-full z-20 p-5">
                        <Link
                          href={item.href || "#"}
                          className="w-full py-3 px-6 bg-gray-900 text-white rounded-3xl font-medium flex items-center justify-center gap-2 hover:bg-black transition-all duration-300"
                        >
                          View all {item.label.toLowerCase()}{" "}
                          <FiArrowUpRight className="text-base" />
                        </Link>
                      </div>
                    )}
                  </div>
                </div>
              ),
          )}
        </div>
      </div>

      {/* --- Mobile Menu Overlay --- */}
      <div
        className={`fixed inset-0 w-screen h-screen z-100 transition-all duration-700 backdrop-blur-xl bg-black/50 ${
          mobileMenu
            ? "opacity-100 pointer-events-auto"
            : "opacity-0 pointer-events-none"
        }`}
      >
        <div
          className="absolute inset-0"
          onClick={() => setMobileMenu(false)}
        />

        <div className="relative w-full h-full flex flex-col p-3 overflow-hidden">
          <div className="w-full h-full bg-white/5 rounded-[40px] flex flex-col p-8 overflow-y-auto max-h-[calc(100vh-24px)] shadow-2xl border border-white/10">
            {/* Header / Dismiss Action */}
            <div className="flex items-center justify-between mb-12">
              <Link
                href="/"
                className="w-36 text-white"
                onClick={() => setMobileMenu(false)}
              >
                <svg
                  className="w-full h-full object-contain fill-current"
                  viewBox="0 0 168 21"
                >
                  {/* SVG Paths for Brand Logo */}
                  <path d="M91.3152 5.40061C91.3152 3.94241 92.5306 2.67359 93.9881 2.67359C95.7162 2.67359 96.797 3.83419 96.797 5.56225H99.7127C99.7127 2.1873 97.3096 0 93.9874 0C90.9371 0 88.3988 2.32257 88.3988 5.42766C88.3988 9.31596 90.883 10.2344 93.9874 11.4221C95.6627 12.07 97.2007 12.5563 97.2007 14.6895C97.2007 16.634 95.9867 18.0651 93.9874 18.0651C91.8813 18.0651 90.7477 16.3905 90.7477 14.446H87.832C87.832 18.0651 90.3426 20.7381 93.9874 20.7381C97.6323 20.7381 100.118 18.2816 100.118 14.6895C100.118 7.10161 91.3145 9.64061 91.3145 5.40061H91.3152Z"></path>
                  <path d="M109.209 4.99609C104.834 4.99609 101.539 8.53405 101.539 12.8539C101.539 17.1737 104.888 20.738 109.155 20.738C112.422 20.738 115.203 18.713 116.337 15.662H113.529C112.718 17.2278 111.017 18.1733 109.262 18.1733C106.806 18.1733 104.915 16.4182 104.348 14.0963H116.743C116.797 13.6371 116.823 13.1508 116.823 12.6922C116.823 8.47926 113.447 4.99609 109.209 4.99609ZM104.348 11.9361C104.509 9.47823 106.751 7.56147 109.181 7.56147C111.611 7.56147 113.853 9.47823 114.014 11.9361H104.348Z"></path>
                  <path d="M127.476 5.40039L123.575 16.0941L119.673 5.40039H116.676L122.617 20.3598H124.588L130.475 5.40039H127.476Z"></path>
                  <path d="M137.942 4.99609C133.567 4.99609 130.273 8.53405 130.273 12.8539C130.273 17.1737 133.621 20.738 137.888 20.738C141.155 20.738 143.936 18.713 145.071 15.662H142.262C141.453 17.2278 139.75 18.1733 137.996 18.1733C135.538 18.1733 133.649 16.4182 133.081 14.0963H145.476C145.53 13.6371 145.556 13.1508 145.556 12.6922C145.556 8.47926 142.182 4.99609 137.942 4.99609ZM133.081 11.9361C133.243 9.47823 135.484 7.56147 137.915 7.56147C140.347 7.56147 142.586 9.47823 142.749 11.9361H133.081Z"></path>
                  <path d="M147.473 8.21195V8.69013V20.3618H150.032V10.1815L167.216 20.3618V17.2405L147.473 5.40039V8.21195Z"></path>
                  <path d="M67.8431 7.50804H67.789C66.6818 5.80635 64.7103 4.99609 62.713 4.99609C58.1775 4.99609 54.7734 8.3981 54.7734 12.935C54.7734 17.4719 58.2296 20.7387 62.713 20.7387C64.7651 20.7387 66.7359 19.8473 67.789 18.0387H67.8431V20.3606H70.652V5.40122H67.8431V7.50804ZM62.686 18.1733C59.823 18.1733 57.5823 15.7168 57.5823 12.9073C57.5823 10.0978 59.7425 7.56079 62.7124 7.56079C65.6822 7.56079 67.8972 9.90973 67.8972 12.9073C67.8972 15.9048 65.6024 18.1733 62.6867 18.1733H62.686Z"></path>
                  <path d="M77.5832 0.378906H74.7736V5.40144H72.75V7.96681H74.7736V20.3608H77.5832V7.96681H80.0403V5.40144H77.5832V0.378906Z"></path>
                  <path d="M18.3089 0.378906H15.5V3.2953H18.3089V0.378906Z"></path>
                  <path d="M18.3089 5.02344H15.5V19.9828H18.3089V5.02344Z"></path>
                  <path d="M25.8409 10.7205C24.8142 10.3959 23.5183 10.0996 23.5183 8.77603C23.5183 7.77639 24.3279 7.18256 25.2728 7.18256C26.4077 7.18256 27.0549 7.91166 27.1895 8.99178H29.9984C29.9443 6.39935 27.9727 4.61719 25.4087 4.61719C22.8447 4.61719 20.7088 6.3723 20.7088 8.93767C20.7088 14.2307 27.5412 12.6102 27.5412 15.743C27.5412 17.0389 26.6227 17.7951 25.381 17.7951C23.707 17.7951 22.9516 16.6074 22.8427 15.0681H20.0352C20.0352 17.417 21.1951 19.2269 23.4094 20.0094C24.0303 20.2252 24.6789 20.3604 25.3262 20.3604C28.1892 20.3604 30.3494 18.5248 30.3494 15.5807C30.3494 12.6366 28.296 11.476 25.8402 10.7205H25.8409Z"></path>
                  <path d="M39.3637 4.61719C34.9891 4.61719 31.6953 8.15514 31.6953 12.475C31.6953 16.7948 35.0432 20.3591 39.3096 20.3591C42.577 20.3591 45.3581 18.3341 46.493 15.2831H43.6842C42.8746 16.8489 41.1722 17.7944 39.4178 17.7944C36.96 17.7944 35.0709 16.0393 34.5028 13.7174H46.8975C46.9516 13.2582 46.978 12.7719 46.978 12.3133C46.978 8.10036 43.6037 4.61719 39.3637 4.61719ZM34.5028 11.5565C34.6651 9.09864 36.9059 7.18188 39.3373 7.18188C41.7688 7.18188 44.0075 9.09932 44.1705 11.5565H34.5028Z"></path>
                  <path d="M9.55945 12.1512C12.1519 11.2327 13.3395 9.09953 13.3395 6.39957C13.3395 4.67151 12.7728 2.88934 11.5046 1.67395C10.0998 0.297591 8.07419 0 6.18314 0H0V19.9826H2.91572V13.8069L13.3389 19.9826V16.8606L6.22575 12.5949L7.61496 12.5293C8.26222 12.5293 8.96359 12.3676 9.55809 12.1512H9.55945ZM4.91499 10.3156H2.91572V2.67359H5.99444C8.317 2.67359 10.4231 3.86192 10.4231 6.40024C10.4231 9.5865 7.50742 10.3156 4.91499 10.3156Z"></path>
                  <path d="M164.759 7.94414L166.061 8.71517V8.08955L165.395 7.69051C165.437 7.68172 165.48 7.66954 165.521 7.65466C165.869 7.53157 166.061 7.24209 166.061 6.84034C166.061 6.57725 165.966 6.33579 165.801 6.17753C165.583 5.9638 165.277 5.93945 165.065 5.93945H164.191V8.63807H164.758V7.94346L164.759 7.94414ZM164.908 7.22856H164.76V6.47715H165.043C165.261 6.47715 165.495 6.57251 165.495 6.84102C165.495 7.10953 165.297 7.22856 164.908 7.22856H164.908Z"></path>
                  <path d="M165.127 10.1622C166.714 10.1622 168 8.87583 168 7.28913C168 5.70242 166.714 4.41602 165.127 4.41602C163.54 4.41602 162.254 5.70242 162.254 7.28913C162.254 8.87583 163.54 10.1622 165.127 10.1622ZM165.127 5.22763C166.264 5.22763 167.189 6.15219 167.189 7.28913C167.189 8.42606 166.264 9.35062 165.127 9.35062C163.99 9.35062 163.066 8.42606 163.066 7.28913C163.066 6.15219 163.99 5.22763 165.127 5.22763Z"></path>
                </svg>
              </Link>
              <button
                onClick={() => setMobileMenu(false)}
                className="w-10 h-10 flex items-center justify-center text-white/80 hover:text-white transition-colors"
              >
                <svg
                  className="w-8 h-8"
                  viewBox="0 0 24 24"
                  fill="none"
                  stroke="currentColor"
                  strokeWidth="2.5"
                  strokeLinecap="round"
                  strokeLinejoin="round"
                >
                  <line x1="18" y1="6" x2="6" y2="18"></line>
                  <line x1="6" y1="6" x2="18" y2="18"></line>
                </svg>
              </button>
            </div>

            {/* --- Mobile Navigation List --- */}
            <nav className="flex flex-col gap-y-1 mb-auto">
              {navItems.map((item) => (
                <div key={item.label} className="w-full">
                  <div className="flex items-center justify-between group">
                    <Link
                      href={item.href || "#"}
                      className="text-white text-[32px] font-bold tracking-tighter leading-[1.1] py-1.5"
                      onClick={() => !item.hasDropdown && setMobileMenu(false)}
                    >
                      {item.label}
                    </Link>
                    {item.hasDropdown && (
                      <button
                        onClick={() =>
                          setMobileAccordion(
                            mobileAccordion === item.id ? null : item.id,
                          )
                        }
                        className={`w-6 h-6 rounded-full border border-white border-solid flex items-center justify-center text-white text-xs transition-all duration-300 ${
                          mobileAccordion === item.id
                            ? "rotate-180"
                            : "rotate-0"
                        }`}
                      >
                        <svg
                          className="w-5 h-5 fill-none stroke-current stroke-3"
                          viewBox="0 0 24 24"
                        >
                          <path d="M6 9l6 6 6-6" />
                        </svg>
                      </button>
                    )}
                  </div>

                  {/* Mobile Accordion Content: Managed via max-height transition for hardware acceleration */}
                  {item.hasDropdown && (
                    <div
                      className={`overflow-hidden transition-all duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] ${
                        mobileAccordion === item.id
                          ? "max-h-200 opacity-100 py-4"
                          : "max-h-0 opacity-0"
                      }`}
                    >
                      <div className="flex flex-col gap-y-3 pb-4">
                        {item.columns
                          .flatMap((c) => c.links)
                          .map((link) => (
                            <Link
                              key={link.id}
                              href={link.href}
                              className="text-white text-[20px] font-bold tracking-tight hover:text-[#B2F6E3] transition-colors py-1 leading-tight"
                              onClick={() => setMobileMenu(false)}
                            >
                              {link.label}
                            </Link>
                          ))}
                      </div>
                    </div>
                  )}
                </div>
              ))}
            </nav>

            {/* --- Mobile CTA --- */}
            <div className="mt-8">
              <Link
                href="/connect-with-us"
                className="w-full h-16 bg-white text-black rounded-full font-bold text-xl flex items-center justify-center gap-2 hover:bg-[#B2F6E3] transition-all duration-300"
                onClick={() => setMobileMenu(false)}
              >
                Get In Touch <FiArrowUpRight className="text-2xl" />
              </Link>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
};

export default Navbar;
